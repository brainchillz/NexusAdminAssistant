...
_DUMMY_HASH = generate_password_hash('naa-dummy-password')
...
def authenticate(username, password):
    ...
    if not rec:
        check_password_hash(_DUMMY_HASH, password or '')  # equalize timing
        return None
    if not check_password_hash(rec['pw_hash'], password or ''):
        return None
    return rec
